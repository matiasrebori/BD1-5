-- @c:\SQL\INSERT_ACADEMICO_MATIAS_MARTINEZ.sql;
-- DDL DESPUES DE LA DEFINICION DE TABLAS , DML DESPUES DE INSERCIONES. MATIAS GABRIEL MARTINEZ REBORI , 5456053
-- Me conecto con un usuario administrador
...
-- Asigno privilegios
grant create session, create table, create any index, create sequence, create view, create trigger to matias;

-- Me desconecto como usuario administrador
disc;
...
--Creacion de estructruras y restricciones
CREATE TABLE ALUMNO (
                ...
                CONSTRAINT PK_COD_ALUMNO PRIMARY KEY (CODIGO));
...
CREATE TABLE CARRERA (
                ...
                ESTADO CHAR(1) NOT NULL,
                CONSTRAINT PK_COD_CARRERA PRIMARY KEY (CODIGO));
...
CREATE TABLE ALUMNO_CARRERA (
                ...
                FECHA_EGRESO DATE,
                ESTADO CHAR(1) NOT NULL,
                CONSTRAINT PK_ALUM_CARRE PRIMARY KEY (COD_ALUMNO, COD_CARRERA),
        CONSTRAINT CARRERAS_ALUMNO_CARRERA_fk FOREIGN KEY (COD_CARRERA) REFERENCES CARRERA (CODIGO),
        CONSTRAINT ALUMNO_ALUMNO_CARRERA_fk FOREIGN KEY (COD_ALUMNO) REFERENCES ALUMNO (CODIGO));

CREATE TABLE MATERIA (
                CODIGO INTEGER NOT NULL,
                NOMBRE VARCHAR2(50) NOT NULL,
                CONSTRAINT PK_COD_MATERIA PRIMARY KEY (CODIGO));
...
CREATE TABLE CARRERA_MATERIA (
                ...
                CONSTRAINT PK_CARR_MATE PRIMARY KEY (COD_MATERIA, COD_CARRERA),
        CONSTRAINT MATERIA_CARRERA_MATERIA_fk FOREIGN KEY (COD_MATERIA) REFERENCES MATERIA (CODIGO),
        CONSTRAINT CARRERA_CARRERA_MATERIA_fk FOREIGN KEY (COD_CARRERA) REFERENCES CARRERA (CODIGO));
...
CREATE TABLE MATRICULACION (
              ...
              --  CALIFICACION INT NOT NULL,
                ...
                CONSTRAINT PK_MATRICULACION PRIMARY KEY (COD_ALUMNO, COD_MATERIA, ANHO, NUM_SEMESTRE),
        CONSTRAINT MATERIA_MATRICULACION_fk FOREIGN KEY (COD_MATERIA) REFERENCES MATERIA (CODIGO),
        CONSTRAINT ALUMNO_MATRICULACION_fk FOREIGN KEY (COD_ALUMNO) REFERENCES ALUMNO (CODIGO));

--a-El campo CI de la tabla alumno debe ser único, no debe admitir duplicaciones. 
ALTER TABLE ALUMNO ADD CONSTRAINT CI_UNIQUE UNIQUE (CI);
--b-Tabla Matriculación: Agregue el campo calificación cuyo tipo de dato es Int, no debe admitir valores nulos y asumir por defecto el valor 0.
ALTER TABLE MATRICULACION ADD CALIFICACION INTEGER DEFAULT 0 NOT NULL;
--c- Tabla Carrera_Materia, la columna costo debe ser mayor a cero. 
ALTER TABLE CARRERA_MATERIA ADD CONSTRAINT COSTO_CHECK CHECK ( COSTO > 0 );
...
--e-El campo 'Es_acreditada' de la tabla Carrera sólo puede tomar los valores S o N y debe asumir por defecto N
ALTER TABLE CARRERA MODIFY ES_ACREDITADA CHAR(1) DEFAULT 'N';
ALTER TABLE CARRERA ADD CONSTRAINT ES_ACREDITADA_CHECK CHECK ( ES_ACREDITADA IN ('S','N') );
--f- Al insertar registros en la tabla matriculación, la columna ‘fecha_matric’ debe asumir por defecto la fecha del sistema.
ALTER TABLE MATRICULACION MODIFY FECHA_MATRIC DATE DEFAULT SYSDATE;
--g- El campo calificación de la tabla matriculación sólo puede tomar valores que estén entre 0 y 5. 
ALTER TABLE MATRICULACION ADD CONSTRAINT MATRICULACION_CHECK CHECK ( CALIFICACION IN (0,1,2,3,4,5) );
...
INSERT INTO CARRERA(CODIGO,NOMBRE,FECHA_CREACION,NUM_RESOLUCION,DURACION,ES_ACREDITADA,ESTADO)
VALUES(SEQ_CARRERA.NEXTVAL,'ING. EN INFORMATICA','30/09/2018','022-003-2018',10,'S','A');
...
INSERT INTO MATRICULACION(COD_ALUMNO,COD_MATERIA,ANHO,NUM_SEMESTRE,FECHA_MATRIC,CALIFICACION,SITUACION)
VALUES(1,9,2018,2,'01/07/2018',3,'C');
INSERT INTO MATRICULACION(COD_ALUMNO,COD_MATERIA,ANHO,NUM_SEMESTRE,FECHA_MATRIC,CALIFICACION,SITUACION)
VALUES(1,16,2018,2,'02/07/2018',4,'C');
INSERT INTO MATRICULACION(COD_ALUMNO,COD_MATERIA,ANHO,NUM_SEMESTRE,FECHA_MATRIC,CALIFICACION,SITUACION)
VALUES(1,17,2018,2,'02/07/2018',5,'C');
INSERT INTO MATRICULACION(COD_ALUMNO,COD_MATERIA,ANHO,NUM_SEMESTRE,FECHA_MATRIC,CALIFICACION,SITUACION)
VALUES(1,18,2018,2,'02/07/2018',2,'C');
INSERT INTO MATRICULACION(COD_ALUMNO,COD_MATERIA,ANHO,NUM_SEMESTRE,FECHA_MATRIC,CALIFICACION,SITUACION)
VALUES(1,19,2018,2,'05/07/2018',3,'C');
...
INSERT INTO MATRICULACION(COD_ALUMNO,COD_MATERIA,ANHO,NUM_SEMESTRE,FECHA_MATRIC,CALIFICACION,SITUACION)
VALUES(1,25,2018,2,'02/07/2018',3,'C');
INSERT INTO MATRICULACION(COD_ALUMNO,COD_MATERIA,ANHO,NUM_SEMESTRE,FECHA_MATRIC,CALIFICACION,SITUACION)
VALUES(1,28,2018,2,'03/07/2018',4,'C');
INSERT INTO MATRICULACION(COD_ALUMNO,COD_MATERIA,ANHO,NUM_SEMESTRE,FECHA_MATRIC,CALIFICACION,SITUACION)
...
-------------------------------------------------------------------------------------------------
/* 2.1 Registre la nueva carrera creada por resolución N° 022-003-2018, los datos se detallan a
continuación. Posteriormente inserte las materias que serán dictadas en el primer y segundo semestre de la
carrera. El costo de las materias es de 25.000 gs por mes. */

---MATERIAS DE LA CARRERA IIN
...
INSERT INTO MATERIA (CODIGO,NOMBRE) VALUES(SEQ_MATERIA.NEXTVAL, 'DESARROLLO DEL EMPRENDEDORISMO');
...
---MATERIAS DE LA CARRERA IIN SEMESTRE IIN
...
--SEMESTRE II IIN
INSERT INTO CARRERA_MATERIA(COD_MATERIA,COD_CARRERA,SEMESTRE,COSTO) VALUES (60,4,2,25000);
...
SELECT * FROM ALUMNO WHERE SEGUNDO_NOMBRE IS NULL OR SEGUNDO_NOMBRE=' ';

/* 2.6 ¿Cuánto costará cursar todas las materias del 2° semestre de la carrera 1 – ‘Técnico Superior
en Electrónica’? Escriba una consulta que realice el cálculo. Obs: Considere que un semestre académico es igual a 4 meses calendario. */

SELECT SUM(COSTO)*4 AS COSTO FROM CARRERA_MATERIA WHERE COD_CARRERA = 1 AND SEMESTRE = 2;

/* 2.7 Muestre el programa de estudios de la carrera 'Programación de computadoras'. El formato del
informe es el siguiente  Obs: El informe debe estar ordenado de manera ascendente por número de semestre. */

CREATE VIEW PROGRAMACION_DE_COMPUTADORAS ( CODIGO_DE_CARRERA, NOMBRE_DE_CARRERA, CODIGO_DE_MATERIA, NOMBRE_DE_MATERIA, SEMESTRE ) 
AS 
SELECT 
        CM.COD_CARRERA,
        C.NOMBRE,
...
INNER JOIN CARRERA C ON CM.COD_CARRERA = C.CODIGO
INNER JOIN MATERIA M ON CM.COD_MATERIA = M.CODIGO
WHERE CM.COD_CARRERA = 2
ORDER BY SEMESTRE ASC;

/* 2.8 Prepare el boletín de calificaciones correspondiente al año 2014; para el alumno con código 3.
Debe mostrar sólo las materias cuyas matriculaciones tengan estado 'C'. Obs: Ordene el boletín de manera ascendente por número de semestre */

CREATE VIEW BOLETIN ( CODIGO_ALUMNO, CI, NOMBRES_Y_APELLIDOS, EDAD, NUMERO_SEMESTRE, ANHO, NOMBRE_MATERIA, CALIFICACION )
AS
SELECT
        MAT.COD_ALUMNO,
        A.CI,
        A.PRIMER_NOMBRE || ' ' || A.SEGUNDO_NOMBRE || ' ' || A.PRIMER_APELLIDO || ' ' || A.SEGUNDO_APELLIDO,
        MONTHS_BETWEEN (SYSDATE, A.FECHA_NACIMIENTO) / 12,
        MAT.NUM_SEMESTRE,
        MAT.ANHO,
        M.NOMBRE,
        MAT.CALIFICACION
FROM MATRICULACION MAT
INNER JOIN ALUMNO A ON MAT.COD_ALUMNO = A.CODIGO
INNER JOIN MATERIA M ON MAT.COD_MATERIA = M.CODIGO
WHERE MAT.ANHO = 2014 AND MAT.COD_ALUMNO=3 AND SITUACION = 'C'
ORDER BY NUM_SEMESTRE ASC;

/* 2.9 Haga una consulta que muestre el promedio de calificaciones de las materias cursadas en el año
2014 por el alumno con código 3. Filtre por las matriculaciones con estado 'C'. */

SELECT AVG(CALIFICACION) FROM MATRICULACION WHERE ANHO = 2014 AND COD_ALUMNO = 3 AND SITUACION = 'C';
